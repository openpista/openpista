...
> WhatsApp은 Telegram과 동일한 HTTP→mpsc 브릿지 패턴을 따릅니다. 어댑터는 HTTP(`axum`)를 통해 웹훅 이벤트를 수신하고, `ChannelEvent`로 변환한 후 `tokio::mpsc`를 통해 전달합니다 — QUIC을 직접 사용하지 **않습니다**.
...
> ContainerTool의 QUIC 클라이언트를 통한 워커 보고 기능은 v0.1.0에서 완전히 구현되었습니다. 게이트웨이 QUIC 서버 제거에 따라 전용 보고 수신 엔드포인트가 필요합니다. 클라이언트 측 코드(`submit_worker_report_over_quic`)는 그대로 유지되며, 이 섹션은 서버 측 수신기와 신뢰성 개선을 추적합니다.

 [ ] 워커 보고 수신 엔드포인트 — 전용 QUIC 리스너(제거된 게이트웨이 QuicServer와 별도, 최소 구성) 또는 `axum`을 통한 HTTP POST 라우트(`/api/worker-report`)로 워커 실행 결과 수신
 [ ] 보고 인증 — 보고 제출 시 워커 작업 토큰 검증 (ContainerTool의 기존 `TaskCredential` 재사용)
 [ ] 보고 확인 응답 프로토콜 — 구조화된 ACK/NACK 응답을 통한 신뢰성 있는 전달
 [ ] 지수 백오프 재시도 — ContainerTool 클라이언트(`submit_worker_report_over_quic` / 향후 HTTP 클라이언트)의 일시적 장애 처리
 [ ] 오프라인 보고 버퍼 — 실패한 보고를 로컬 디스크(`~/.openpista/report-queue/`)에 큐잉, 연결 복구 시 재전송
 [ ] HTTP 보고 클라이언트 옵션 — ContainerTool의 기존 QUIC 클라이언트와 함께 프로토콜 대안으로 `submit_worker_report_over_http()` 추가
