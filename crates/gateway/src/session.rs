...
mod tests {
    ...
    fn build_quic_harness() -> QuicHarness {
        ...
        let server_endpoint =
            quinn::Endpoint::server(server_config, "127.0.0.1:0".parse().expect("server bind"))
        ...
        let mut client_endpoint =
            quinn::Endpoint::client("127.0.0.1:0".parse().expect("bind")).expect("client endpoint");
    ...
    #[tokio::test]
    async fn handle_stream_roundtrip_returns_handler_text() {
        let QuicHarness {
            server_endpoint,
            client_endpoint,
            server_addr,
        ...
        let handler: AgentHandler =
            Arc::new(|event| Box::pin(async move { Some(format!("echo:{}", event.user_message)) }));
        ...
        let server_task = tokio::spawn(async move {
            ...
            let result = handle_stream(&mut recv, &mut send, handler).await;
        ...
        let mut resp = vec![0_u8; len];
        recv.read_exact(&mut resp).await.expect("read response");
        assert_eq!(String::from_utf8(resp).expect("utf8"), "echo:hello");
    ...
    #[tokio::test]
    async fn handle_stream_roundtrip_uses_default_ok_for_none() {
        let QuicHarness {
            server_endpoint,
            client_endpoint,
            server_addr,
        ...
        let handler: AgentHandler = Arc::new(|_event| Box::pin(async move { None }));
        ...
        let mut resp = vec![0_u8; len];
        recv.read_exact(&mut resp).await.expect("read response");
        assert_eq!(String::from_utf8(resp).expect("utf8"), "OK");
