...
mod tests {
    ...
    #[test]
    fn web_adapter_config_fields_are_accessible() {
        let config = WebAdapterConfig {
            port: 3210,
            token: "secret".to_string(),
            cors_origins: "http://localhost".to_string(),
            static_dir: "/tmp/web".to_string(),
        };
        assert_eq!(config.port, 3210);
        assert_eq!(config.token, "secret");
        assert_eq!(config.cors_origins, "http://localhost");
        assert_eq!(config.static_dir, "/tmp/web");
    }

    #[test]
    fn web_adapter_constructor_and_channel_id() {
        let config = WebAdapterConfig {
            port: 3210,
            token: "tok".to_string(),
        ...
        let adapter = WebAdapter::new(config, resp_tx);
        assert_eq!(adapter.channel_id().as_str(), "web:ws");
        assert_eq!(adapter.port, 3210);
        assert_eq!(adapter.token, "tok");
    }

    #[test]
    fn clone_for_responses_shares_client_map() {
        let config = WebAdapterConfig {
            port: 3210,
            token: "tok".to_string(),
            cors_origins: String::new(),
        ...
        let (resp_tx, _resp_rx) = mpsc::channel(1);
        let adapter = WebAdapter::new(config, resp_tx);
        let clone = adapter.clone_for_responses();
        assert!(Arc::ptr_eq(&adapter.clients, &clone.clients));
        assert_eq!(clone.port, adapter.port);
        assert_eq!(clone.token, adapter.token);
    }

    #[tokio::test]
    async fn send_response_delivers_to_registered_client() {
        let config = WebAdapterConfig {
            port: 3210,
            token: String::new(),
            cors_origins: String::new(),
        ...
        adapter.clients.insert("test-client".to_string(), client_tx);

        let resp = AgentResponse::new(
            ChannelId::from("web:test-client"),
            SessionId::from("s1"),
            "hello",
        );
        adapter
    ...
    #[tokio::test]
    async fn send_response_returns_error_for_unknown_client() {
        let config = WebAdapterConfig {
            port: 3210,
            token: String::new(),
            cors_origins: String::new(),
        ...
        let resp = AgentResponse::new(
            ChannelId::from("web:nonexistent"),
            SessionId::from("s1"),
            "hello",
        );
        let err = adapter.send_response(resp).await.expect_err("should fail");
        assert!(err.to_string().contains("not connected"));
    }

    #[tokio::test]
    async fn send_response_returns_error_for_empty_channel_id() {
        let config = WebAdapterConfig {
            port: 3210,
            token: String::new(),
            cors_origins: String::new(),
        ...
        let resp = AgentResponse::new(ChannelId::from("web:"), SessionId::from("s1"), "hello");
        let err = adapter.send_response(resp).await.expect_err("should fail");
        assert!(err.to_string().contains("Empty client id"));
    }

    #[test]
    fn parse_ws_message_handles_empty_string() {
        assert_eq!(parse_ws_message(""), "");
    }

    #[test]
    fn parse_ws_message_handles_null_user_message() {
        let msg = r#"{"user_message": null}"#;
        assert_eq!(parse_ws_message(msg), msg);
    }

    #[test]
    fn build_cors_layer_with_invalid_origin_skips_it() {
        // Invalid origins are silently filtered out
        let _ = build_cors_layer("not-a-url,http://valid.com");
    }

    #[test]
    fn parse_client_id_with_only_prefix() {
        // "web:" -> empty client id -> error
        assert!(parse_client_id("web:").is_err());
        // bare empty string -> error
